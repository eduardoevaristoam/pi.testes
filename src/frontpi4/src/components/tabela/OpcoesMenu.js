  ...
  const handleExcluir = async (event) => {
    ...
    try {
      const response = await fetch(
        `https://api-p-i-4.onrender.com/${direction}/${Id}`,
        {
          method: "DELETE",
        }
      );
    ...
    <div className="modal">
      <div className="modal-content">
        <div className="modal-buttons">
          {direction !== "devices" && (
            <button type="button" onClick={handleAtribuirServico}>
              Atribuir Serviço
            </button>
          )}
      ...
      {openModal === "editmodaldv" && (
        <EditModalDV Id={Id} isOpen={true} onClose={closeModalHandler} />
      )}
      {openModal === "editmodalpl" && (
        <EditModalPL Id={Id} isOpen={true} onClose={closeModalHandler} />
      )}
      {openModal === "atribuirdv" && (
        <AtribuirDV Id={Id} isOpen={true} onClose={closeModalHandler} />
      )}
      {openModal === "atribuirpl" && (
        <AtribuirPL Id={Id} isOpen={true} onClose={closeModalHandler} />
      )}
      {openModal === "atribuirfs" && (
        <AtribuirFS Id={1} isOpen={true} onClose={closeModalHandler} />
      )}
    </div>
  );
}
